...
\begin{tikzpicture}[align=center,node distance = 1cm]
  ...
  \node (whammy) [processor, right = of preamp] {Whammy};
  \node (delay) [processor, right = of whammy] {Delay};
  \node (speaker) [source, right = of delay] {Amp/PA};
  \draw [cable] (bsn) -- (lj);
  \draw [cable] (lj) -- (preamp);
  \draw [cable] (preamp) -- (whammy);
  \draw [cable] (whammy) -- (delay);
  \draw [cable] (delay) -- (speaker);
\end{tikzpicture}
