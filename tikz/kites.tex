...
\begin{tikzpicture}[align=center,node distance = 1cm]
  ...
  \node (organ) [source, below= of bsn] {Keyboard};
  ...
  \node (loop) [processor, text width = 3cm, right = of mix] {Looper Pedal};
  \draw [cable] (mix) -- (loop);
  \draw [cable] (loop) to[out=-90, in=-45] (mix);
  ...
  \draw [cable] (organ) to[out=0, in=240] (mix);
\end{tikzpicture}
