...
\begin{tikzpicture}[align=center,node distance = 1cm]
  \node (cbsn) [source] {Contrabassoon\\ (optional)};
  \node (bsn) [source, right= of cbsn] {Bassoon};
  \node (orch) [source, right= of bsn, xshift=2.3cm] {Wind Ensemble;
  \node (lj) [input, below= of bsn] {Little-Jake};
  \node (lj2) [input, below= of cbsn] {Little-Jake};
  \node (preamp) [routing, below = of lj] {Preamp};
  \node (effects) [processor, right = of preamp, xshift=1cm] {Effects Chain};
  \draw node[anchor = north, below = of effects, yshift=1cm] {Including Tap Delay and \\ Octave Pedal if no Contrabassoon};
  \node (speaker) [source, above = of effects, xshift=-2cm] {Amp};
  \node (speaker2) [source, above = of effects, xshift=2cm] {Amp};
  \draw [cable] (cbsn) -- (lj2);
  \draw [cable] (bsn) -- (lj);
  \draw [cable] (lj) -- (preamp);
  \draw [cable] (lj2) -- (preamp);
  \draw [cable] (preamp) -- (effects);
  \draw [cable] (effects) -- (speaker);
  \draw [cable] (effects) -- (speaker2);
\end{tikzpicture}
